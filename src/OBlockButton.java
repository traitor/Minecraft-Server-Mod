...
public class OBlockButton extends OBlock {
    ...
    public boolean a(OWorld paramOWorld, int paramInt1, int paramInt2, int paramInt3, OEntityPlayer paramOEntityPlayer) {
        ...
        int change = (Integer) etc.getLoader().callHook(PluginLoader.Hook.REDSTONE_CHANGE, new Block(bi, paramInt1, paramInt2, paramInt3), 0, 1);
        if (change == 0)
            return true;
    ...
    public void a(OWorld paramOWorld, int paramInt1, int paramInt2, int paramInt3, Random paramRandom) {
        ...
        if ((i & 0x8) == 0)
            return;
        // hMod: Allow button to provide power
        int change = (Integer) etc.getLoader().callHook(PluginLoader.Hook.REDSTONE_CHANGE, new Block(bi, paramInt1, paramInt2, paramInt3), 1, 0);
        if (change > 0)
            return;
