...
public class fx extends jy
{
	...
	public void E() {
		 ...
		 */
		if ((etc.getInstance().autoHeal()==PluginLoader.HookResult.DEFAULT_ACTION)
				&& (this.l.k == 0) && (this.aQ < 20) && (this.X % 20 * 4 == 0)){
			a(1);
		}
		else if ((etc.getInstance().autoHeal()==PluginLoader.HookResult.ALLOW_ACTION)
				&& (this.aQ < 20) && (this.X % 20 * 4 == 0)){
			a(1);
		}
		...
		if (f1 > 0.1F) f1 = 0.1F;
		if ((!this.A) || (this.aQ <= 0)) f1 = 0.0F;
		if ((this.A) || (this.aQ <= 0)) f2 = 0.0F;
	...
	{
		this.bn = 0;
		if (this.aQ <= 0) return false;

		if ((paramdx instanceof gb) || (paramdx instanceof dw)) {
			if (this.l.k == 0) paramInt = 0;
			if (this.l.k == 1) paramInt = paramInt / 3 + 1;
			if (this.l.k == 3) paramInt = paramInt * 3 / 2;
		}
